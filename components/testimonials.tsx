        ...
        >
          <div className="flex gap-6 overflow-x-auto scrollbar-hide snap-x snap-mandatory scroll-smooth pb-8 px-4">
            {testimonials.map((testimonial, index) => (
              ...
              >
                <div className="bg-slate-50 rounded-2xl p-8 h-full shadow-md hover:shadow-xl transition-all duration-300 hover:-translate-y-2">
                  <div className="flex gap-1 mb-4">
                    {[...Array(testimonial.rating)].map((_, i) => (
                      <Star key={i} className="h-5 w-5 fill-secondary text-secondary" />
                    ))}
                  </div>
                  <p className="text-slate-700 mb-4 leading-relaxed text-lg italic">"{testimonial.quote}"</p>
