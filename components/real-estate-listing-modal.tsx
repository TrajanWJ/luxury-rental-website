...
interface RealEstateListingModalProps {
  property: Property
  onClose: () => void
}

export function RealEstateListingModal({ property, onClose }: RealEstateListingModalProps) {
  usePopupFreeze(true)
    ...
    >
      ...
      >
        ...
        >
          ...
          {/* Golden Sunset Filter for Milan Manor */}
          {property.name === "Milan Manor" && (
            <div className="absolute inset-0 bg-[#FFD700]/10 mix-blend-overlay pointer-events-none z-[5]" />
          )}

          {/* For Sale Badge */}
          <div className="absolute left-4 top-4 z-[6] rounded-full border border-[#E7D6C1]/45 bg-[#9D5F36] px-4 py-1.5 text-[11px] font-bold uppercase tracking-[0.14em] text-[#F8F1E8]">
            For Sale
          </div>
        ...
        <div className="p-4 sm:p-6 md:p-8">
          ...
          <div className="flex flex-col sm:flex-row gap-3">
            <Button
              ...
              onClick={() => {
                onClose()
                setTimeout(() => openContactModal("Milan Manor \u2013 Real Estate"), 300)
              }}
            >
              Inquire Now
            </Button>
